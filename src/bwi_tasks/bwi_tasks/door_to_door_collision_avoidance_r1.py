...
class BWIbot(Node):
    ...
    def __init__(self,
        ...
        super().__init__('r1_bwibot_navigator')
        ...
        self.current_goal.header.frame_id = 'r1/map'
        ...
        self._nav_client = ActionClient(
            self, NavigateToPose, 'r1/navigate_to_pose',
        ...
        self._compute_path_client = ActionClient(
            self, ComputePathToPose, 'r1/compute_path_to_pose',
        ...
        self.clear_costmaps_client = self.create_client(
            Empty, '/r1/global_costmap/clear_entirely_global_costmap',
            callback_group=self.service_callback_group
        )
        self.clear_local_costmap_client = self.create_client(
            Empty, '/r1/local_costmap/clear_entirely_local_costmap',
            callback_group=self.service_callback_group
        )
        self.clear_hallucination_client = self.create_client(
            Empty, '/r1/clear_virtual_circles',
            callback_group=self.service_callback_group
        )
    ...
    def send_goal_and_wait(self, x: float, y: float, yaw: float, timeout_sec: float = 60.0):
        ...
        goal_msg.pose.header.frame_id = 'r1/map'
    ...
    def generate_vo(self, center: np.ndarray, radius: float, dr: float,
        ...
        self.get_logger().info(f"The Hack")
        ...
        self.phhp = True
        self.get_logger().info(f"Generated {len(vos)} virtual obstacles for r1")

    def wait_for_r2_pose(self, timeout_sec: float = 30.0):
        """
        Block until one message arrives on /r2/amcl_pose and return (x, y, yaw).
        ...
        sub = self.create_subscription(
            PoseWithCovarianceStamped, '/r2/amcl_pose', _cb, amcl_qos,
            callback_group=self.nav_callback_group,
        )

        self.get_logger().info("Waiting for /r2/amcl_pose to set goal...")
        if not event.wait(timeout=timeout_sec):
            self.get_logger().error("Timed out waiting for /r2/amcl_pose — using fallback pose")
            self.destroy_subscription(sub)
            return None

        self.destroy_subscription(sub)
        x, y, yaw = result[0]
        self.get_logger().info(f"r2 initial pose captured: x={x:.3f}, y={y:.3f}, yaw={yaw:.4f}")
        return x, y, yaw
...
def main(args=None):
    ...
    try:
        ...
        goal = robot.wait_for_r2_pose(timeout_sec=max(0.1, remaining))
        if goal is None:
            ...
            _write_episode_result(
                ...
                'r1',
                ttd,
                _normalize_status(status, collision_state, exception_obj),
                collision_state,
                parsed.phhp_radius,
                parsed.phhp_dr,
                parsed.phhp_k_begin,
